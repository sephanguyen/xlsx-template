...
module.exports = (function () {
  ...
  Workbook.prototype._rebuild = function () {
    ...
    const xmlFiles = self.workbookRels.findall('*').sort(function (rel1, rel2) {
      //using order
    ...
    xmlFiles.forEach(function (item, index) {
      item.attrib.Id = 'rId' + (index + 1);
    });

    const firstIndexOfSheet = xmlFiles.findIndex(
      (element) => path.basename(element.attrib.Type) === 'worksheet'
    );
    const sheetFiles = self.workbook.findall('sheets/sheet');
    sheetFiles.forEach(function (item, index) {
      item.attrib['r:id'] = 'rId' + (firstIndexOfSheet + (index + 1));
